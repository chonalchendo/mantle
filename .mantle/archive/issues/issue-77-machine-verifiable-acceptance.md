...
- [x] ac-01: Issue frontmatter supports an `acceptance_criteria` list where each entry has `id`, `text`, and `passes: true|false`; Pydantic schema validates it.
- [x] ac-02: Markdown AC checkboxes in the issue body are generated from the structured list on save, so the prose stays true to the data.
- [x] ac-03: `/mantle:verify` flips `passes` per AC via a dedicated CLI operation (not raw edit) and emits a report of ACs still at `passes: false`.
- [x] ac-04: `/mantle:review` refuses to approve an issue unless every AC is `passes: true` or carries an explicit waiver.
- [x] ac-05: A one-time `mantle migrate-acs` CLI backfills existing planned/implemented/verified issues from their markdown checkboxes into structured frontmatter.
- [x] ac-06: Unit tests cover the Pydantic schema, frontmatter round-trip, migration, body-sync rendering, and the flip-passes CLI behavior.
